...
describe("runAgentLoop", () => {
  ...
  it("executes a tool call and feeds the result back", async () => {
    const echo = {
      ...
      description: "echo",
      parameters: {},
      execute: vi.fn(async (a: any) => `did:${a.x}`),
    };
    mocked()
      .mockResolvedValueOnce({
        text: "",
        toolCalls: [{ callId: "c1", name: "echo", arguments: JSON.stringify({ x: "hi" }) }],
      })
  ...
  it("reports an unknown tool without throwing", async () => {
    mocked()
      .mockResolvedValueOnce({
        text: "",
        toolCalls: [{ callId: "c1", name: "missing", arguments: "{}" }],
      })
